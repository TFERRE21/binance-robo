...
/* =========================================================
   CRIPTOPRO — ROBÔ
   CONFIGURAÇÃO -> SALVAR -> START -> TERMO -> INICIAR
========================================================= */
...
function robotSetMessage(message){
  const el=document.getElementById("robotConfigMessage");
  if(el) el.textContent=message || "";
}

function robotSetStatus(text){
  const el=document.getElementById("robotConfigStatus");
  if(el) el.textContent=text || "";
}

function robotPayload(){
  return {
    accountId: document.getElementById("robotAccount").value,
...
async function carregarContasRobo(){
  const select=document.getElementById("robotAccount");
  if(!select)return;
  ...
  try{
    ...
    if(!accounts.length){
      select.innerHTML='<option value="">Nenhuma conta Binance cadastrada</option>';
      document.getElementById("robotSaveConfigBtn").disabled=true;
      return;
    }
    ...
    accounts.forEach(function(account){
      ...
      option.textContent=account.name || account.nome || ("Conta "+account.id);
      select.appendChild(option);
    });

    if(typeof contaSelecionada!=="undefined"){
      const old=accounts.find(a=>String(a.id)===String(contaSelecionada));
      if(old)select.value=old.id;
    }
  }catch(error){
    select.innerHTML='<option value="">Erro ao carregar contas</option>';
    robotSetMessage("Não foi possível carregar as contas Binance: "+error.message);
  }
}
...
async function salvarConfiguracaoRobo(){
  const token=robotToken();
  if(!token){
    robotSetMessage("Sua sessão expirou. Faça login novamente.");
    return;
  }
  ...
  if(!payload.accountId){
    robotSetMessage("Selecione uma conta Binance.");
    return;
  }

  const button=document.getElementById("robotSaveConfigBtn");
  button.disabled=true;
  robotSetMessage("Salvando configuração...");
  ...
  try{
    ...
    if(!robotCurrentConfigId){
      throw new Error("O servidor salvou a configuração, mas não retornou o ID da configuração.");
    }

    robotRiskConfigId=robotCurrentConfigId;

    document.getElementById("robotStartBtn").disabled=false;
    robotSetStatus("CONFIGURAÇÃO SALVA");
    robotSetMessage("Configuração salva. Clique em START ROBÔ para ler e aceitar o termo.");
  }catch(error){
    robotSetMessage("Erro ao salvar configuração: "+error.message);
  }finally{
...
function abrirTermoRisco(configId){
  robotRiskConfigId=configId||robotCurrentConfigId||null;

  const modal=document.getElementById("robotRiskModal");
  const checkbox=document.getElementById("robotRiskAccepted");

  if(!modal){
    robotSetMessage("O Termo de Responsabilidade não foi encontrado na página.");
    return;
  }
...
async function aceitarTermoRisco(){
  ...
  if(!checkbox || !checkbox.checked){
    alert("Para continuar, marque a opção confirmando que leu, compreendeu e aceita o Termo de Responsabilidade e Ciência de Riscos.");
    return;
  }
  ...
  const configId=robotRiskConfigId||robotCurrentConfigId;

  if(!configId){
    alert("Nenhuma configuração válida foi encontrada. Salve a configuração novamente.");
    return;
  }
  ...
  try{
    ...
    fecharTermoRisco();
    await iniciarRoboDepoisDoTermo();
  }catch(error){
    console.error("ERRO AO ACEITAR TERMO:",error);
    alert("Não foi possível registrar o termo.\n\n"+error.message);
  }finally{
    if(button){
      button.disabled=false;
...
async function iniciarRoboDepoisDoTermo(){
  const token=robotToken();
  const configId=robotRiskConfigId||robotCurrentConfigId;

  if(!token)throw new Error("Sessão expirada.");
  if(!configId)throw new Error("Configuração do robô não encontrada.");
  ...
  robotSetStatus("ROBÔ ATIVO");
  robotSetMessage("Robô iniciado com a configuração salva.");
}

async function iniciarRoboComTermo(){
  if(!robotCurrentConfigId){
    robotSetMessage("Primeiro salve a configuração do robô.");
    return;
  }
...
async function pararRobo(){
  ...
  try{
    ...
    robotSetStatus("ROBÔ PARADO");
    robotSetMessage("Robô parado.");
  }catch(error){
    robotSetMessage("Erro ao parar o robô: "+error.message);
  }
}

document.addEventListener("DOMContentLoaded",function(){
  const save=document.getElementById("robotSaveConfigBtn");
  const start=document.getElementById("robotStartBtn");
  const stop=document.getElementById("robotStopBtn");

  if(save)save.addEventListener("click",salvarConfiguracaoRobo);
  if(start)start.addEventListener("click",iniciarRoboComTermo);
  if(stop)stop.addEventListener("click",pararRobo);
...
/* CRIPTOPRO — CONTROLE DO ROBÔ */
.robotConfigSection{margin-top:22px}
.robotConfigBox{padding:20px;border:1px solid var(--line);border-radius:16px;background:rgba(10,18,31,.72)}
.robotConfigGrid{display:grid;grid-template-columns:repeat(4,minmax(150px,1fr));gap:14px}
.robotField{display:flex;flex-direction:column;gap:7px}
.robotField label{font-size:11px;font-weight:900;color:#aebbd0}
.robotField input,.robotField select{width:100%;box-sizing:border-box;padding:11px 12px;border-radius:10px;border:1px solid #263852;background:#091321;color:#fff;outline:none}
.robotField input:focus,.robotField select:focus{border-color:#3e83d7}
.robotConfigCheck{margin-top:14px;font-size:12px;color:#c4cede}
.robotConfigCheck input{margin-right:7px}
.robotConfigActions{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}
.robotBtn{border:1px solid transparent;border-radius:10px;padding:11px 16px;font-size:11px;font-weight:950;cursor:pointer}
.robotBtn:disabled{opacity:.45;cursor:not-allowed}
.robotBtnSave{background:#fff;color:#07111e}
.robotBtnStart{background:#16835e;color:#fff;border-color:#20a879}
.robotBtnStop{background:#812534;color:#fff;border-color:#b33a4a}
.robotConfigStatus{padding:8px 12px;border-radius:999px;background:#273348;color:#aebbd0;font-size:10px;font-weight:950;white-space:nowrap}
.robotConfigMessage{min-height:20px;margin-top:12px;color:#aebbd0;font-size:12px}
@media(max-width:1000px){.robotConfigGrid{grid-template-columns:repeat(2,minmax(150px,1fr))}}
@media(max-width:600px){.robotConfigGrid{grid-template-columns:1fr}.robotConfigActions{flex-direction:column}.robotBtn{width:100%}.robotConfigStatus{display:none}}
...
<div class="container">
  ...
  <!-- =========================================================
       CRIPTOPRO — CONFIGURAÇÃO DO ROBÔ
  ========================================================= -->
  <section class="section robotConfigSection">

    <div class="sectionHead">
      <div>
        <h2 class="sectionTitle">🤖 Configuração do Robô</h2>
        <div class="sectionDesc">
          Configure o robô, salve os parâmetros e depois pressione START para ler e aceitar o Termo de Responsabilidade.
        </div>
      </div>
      <div id="robotConfigStatus" class="robotConfigStatus">CONFIGURAÇÃO NÃO SALVA</div>
    </div>

    <div class="robotConfigBox">

      <div class="robotConfigGrid">

        <div class="robotField">
          <label for="robotAccount">Conta Binance</label>
          <select id="robotAccount">
            <option value="">Carregando contas...</option>
          </select>
        </div>

        <div class="robotField">
          <label for="robotStrategy">Estratégia</label>
        ...
        <div class="robotField">
          <label for="robotMaxCoins">Máximo de moedas</label>
          <input id="robotMaxCoins" type="number" min="1" max="50" step="1" value="20">
        </div>

      </div>

      <div class="robotConfigCheck">
        <label>
      ...
      <div class="robotConfigActions">
        <button type="button" id="robotSaveConfigBtn" class="robotBtn robotBtnSave">
          💾 SALVAR CONFIGURAÇÃO
        </button>

        <button type="button" id="robotStartBtn" class="robotBtn robotBtnStart" disabled>
          ▶ START ROBÔ
        </button>

        <button type="button" id="robotStopBtn" class="robotBtn robotBtnStop">
          ⏹ STOP
        </button>
      </div>
